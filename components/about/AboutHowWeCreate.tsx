...
const RU: typeof EN = {
  eyebrow: 'От идеи до семьи',
  headline: 'Каждый опыт TALIMOON начинается с одной мысли.',
  statement: 'Мы не хотим создавать для детей то, что является просто «достаточно хорошим».',
  steps: [
    { n: '01', name: 'Знакомство', body: 'Сначала мы понимаем, для кого создаём. В именных историях это сам ребёнок: его интересы, мечты, характер и то, что важно для его семьи.' },
    { n: '02', name: 'Смысл', body: 'Затем мы решаем, что этот опыт должен дать ребёнку. Он должен быть не просто увлекательным, а значимым для ребёнка и его семьи.' },
    { n: '03', name: 'История', body: 'Смысл превращается в приключение: историю, за которой ребёнок хочет следить и которую он запоминает.' },
    { n: '04', name: 'Образ', body: 'Воображение становится видимым миром. Каждое изображение должно усиливать историю и обогащать воображение ребёнка.' },
    { n: '05', name: 'Опыт', body: 'Формат служит цели. Чтение, прослушивание, просмотр или игра: какую бы форму это ни принимало, в центре всегда остаётся опыт ребёнка.' },
    { n: '06', name: 'Проверка', body: 'Мы снова смотрим на результат: глазами ребёнка и глазами родителя. Мы заново проверяем смысл, чувство и сам опыт.' },
    { n: '07', name: 'Воплощение', body: 'То, что мы создали, доходит до настоящей семьи. Ведь для TALIMOON работа не заканчивается на экране или в макете: она обретает смысл, когда ребёнок держит её в руках, читает, слушает или чувствует.' },
  ],
};
