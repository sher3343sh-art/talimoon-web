...
function Icon({ name }: { name: 'play' | 'pause' | 'sound' | 'mute' | 'prev' | 'next' | 'full' | 'close' | 'restart' }) {
  ...
  if (name === 'restart') return <svg {...common} fill="none" stroke="currentColor" strokeWidth="1.7" strokeLinecap="round" strokeLinejoin="round"><path d="M4 4v6h6M5.6 15.5A8 8 0 1 0 6 7.4L4 10" /></svg>;
...
export function Reader({ slug }: { slug: string }) {
  ...
  const restart = useCallback(() => {
    ...
    setPhase('front');
    setChrome(true);
    saveProgress({ slug, page: 0, mode: narration ? 'storytime' : 'self', atISO: new Date().toISOString() });
  }, [clearTimer, narration, slug]);

  const toggleFullscreen = useCallback(async () => {
    const mobileDocument = document as Document & {
    ...
    const fullscreenElement = document.fullscreenElement ?? mobileDocument.webkitFullscreenElement;
    try {
      if (!fullscreenElement) {
        if (element?.requestFullscreen) await element.requestFullscreen({ navigationUI: 'hide' });
          ...
          setNeedsRotate(window.matchMedia('(orientation: portrait)').matches);
          return;
        }
        setIsFullscreen(true);
        try {
      ...
      else await mobileDocument.webkitExitFullscreen?.();
    } catch {
      setIsFullscreen(true);
      setNeedsRotate(window.matchMedia('(orientation: portrait)').matches);
    }
  ...
  const exit = useCallback(() => {
    clearTimer(); narrationRef.current?.pause();
    const mobileDocument = document as Document & { webkitFullscreenElement?: Element | null; webkitExitFullscreen?: () => Promise<void> | void };
    if (document.fullscreenElement && document.exitFullscreen) document.exitFullscreen().catch(() => undefined);
    else if (mobileDocument.webkitFullscreenElement) Promise.resolve(mobileDocument.webkitExitFullscreen?.()).catch(() => undefined);
    router.push(`/story-library/s/${slug}`);
  }, [clearTimer, router, slug]);
  ...
  useEffect(() => {
    ...
    return () => { document.removeEventListener('fullscreenchange', update); document.removeEventListener('webkitfullscreenchange', update); window.removeEventListener('orientationchange', update); };
  }, []);
  useEffect(() => {
